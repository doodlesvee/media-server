...
export type Appearance = {
  ...
  /**
   * Height of the full-bleed banners — the homepage hero and the performer
   * and studio headers — as a percentage of the viewport.
   *
   * One value for both rather than a slider each: they're the same kind of
   * thing, and two controls only offer the chance to make them disagree.
   */
  bannerHeight: number;
  ...
  useEffect(() => {
    ...
    fetchAppearance()
      .then((stored) => {
        ...
        setValue((current) => ({ ...current, ...stored }));
      })
      .catch(() => {
